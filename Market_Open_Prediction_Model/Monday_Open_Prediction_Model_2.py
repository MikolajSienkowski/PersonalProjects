...
def get_data(ticker=TICKER):
    ...
    eth = yf.download('ETH-USD', start=start, end=end, interval='1d', auto_adjust=True)

    df.columns = df.columns.get_level_values(0)
    btc.columns = btc.columns.get_level_values(0)
    eth.columns = eth.columns.get_level_values(0)

    return df.dropna(), btc.dropna(), eth.dropna()

def add_feature_and_target(df, btc, eth):
    df['Target'] = (df['Open'].shift(-1) - df['Close']) / df['Close']
    btc['Feature BTC'] = (btc['Close'].shift(-2) - btc['Close']) / btc['Close']
    eth['Feature ETH'] = (eth['Close'].shift(-2) - eth['Close']) / eth['Close']

    df = df[df.index.dayofweek == 4].copy()
    btc = btc[btc.index.dayofweek == 4].copy()
    eth = eth[eth.index.dayofweek == 4].copy()

    df['Mean of Target'] = df['Target'].mean()

    df = pd.concat([df, btc, eth], axis=1).dropna()
...
def main():
    df, btc, eth = get_data()
    df = add_feature_and_target(df, btc, eth)
